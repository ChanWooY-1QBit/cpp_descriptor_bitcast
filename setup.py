...
class CMakeBuildExt(build_ext):
    def build_extensions(self):
        ...
        import platform, sys, distutils.sysconfig, pybind11

        # Work out the relevant Python paths to pass to CMake, adapted from the
        # PyTorch build system
        if platform.system() == "Windows":
            cmake_python_library = "{}/libs/python{}.lib".format( distutils.sysconfig.get_config_var("prefix"), distutils.sysconfig.get_config_var("VERSION"),)
            if not os.path.exists(cmake_python_library):
                cmake_python_library = "{}/libs/python{}.lib".format( sys.base_prefix, distutils.sysconfig.get_config_var("VERSION"),)
        else:
        ...
        cmake_args = [
            ...
            "-DABSEIL_ROOT='/home/chanwooyang/abseil-cpp'"
        ]
    ...
    def build_extension(self, ext):
        ...
        subprocess.check_call(
            ["sudo", "cmake", "--build", ".", "--target", target_name],
            cwd = self.build_temp,
        )

setup(
    name = "einsum_bit_cast",
    author = "Chan Woo Yang",
    author_email = "chanwoo.yang@1qbit.com",
    url = "https://github.com/ChanWooY-1QBit/cpp_descriptor_bitcast.git",
    license = "MIT",
    description = (
        "test c++ bit_cast"
    ),
